...
if (isset($_POST['berdasarkan']) && !empty($_POST['berdasarkan']) && isset($_POST['isi']) && !empty($_POST['isi']))
{
	...
	if (isset($_POST['dari']) && !empty($_POST['dari']) && isset($_POST['sampai']) && !empty($_POST['sampai']))
	{
		...
		$query="SELECT * FROM data_like_berita where $berdasarkan like '%$isi%'";
	}
	else
	{
		$query="SELECT * FROM data_like_berita where $berdasarkan like '%$isi%'";
...
{
	$query = "select * from data_like_berita";
}
...
$proses = mysql_query($query);	
  while($data = mysql_fetch_array($proses))
  {
	...
	$id_like_berita = $data["id_like_berita"];	
    $hasil['id_like_berita'] = $id_like_berita;
	$hasil['tanggal'] = $data["tanggal"];
	$hasil['id_berita'] = $data["id_berita"];
